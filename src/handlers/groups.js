...
module.exports = (bot) => {
  ...
  bot.hears(MENU.createFromList, async (ctx) => {
    ...
    if (!a?.authed) return ctx.reply('🚫 *Akses Ditolak.* Silakan hubungkan akun terlebih dahulu.', { parse_mode: 'Markdown' });
    ctx.session = { act: 'create_from_list' };
    
    const msg = 
`📝 *PEMBUATAN GRUP MASAL (BATCH)*

Kirimkan daftar nama grup (satu nama per baris).
Maksimal: *${MAX_GROUPS_PER_BATCH} grup* per proses.
...
Tim Pemasaran\``;
  ...
  bot.hears(MENU.createSequential, async (ctx) => {
    ...
    await ctx.reply('🔡 *Masukkan Nama Dasar (Prefix):*\n\n_Contoh: Jika input "Server", hasilnya: Server 1, Server 2..._', { 
      reply_markup: inlineCancelKb(),
      parse_mode: 'Markdown'
    });
  });
  ...
  bot.hears(MENU.lastResult, async (ctx) => {
    const a = getAcc(ctx.from.id);
    if (!a) return ctx.reply('❌ Tidak ada sesi aktif.');
    const list = a.lastResults || [];
    if (!list.length) return ctx.reply('ℹ️ *Log Kosong.* Belum ada riwayat pembuatan grup.', { parse_mode: 'Markdown' });
    
    const txt = list.map((x,i)=> `• ${x.title} — [Link Masuk](${x.link})`).join('\n');
    await ctx.reply(`📂 *LOG AKTIVITAS TERAKHIR:*\n\n${txt}`, { parse_mode: 'Markdown', disable_web_page_preview: true });
  });
  ...
  bot.on('message:text', async (ctx, next) => {
    ...
    if (!s) return next();
    const a = getAcc(ctx.from.id);
    if (!a?.authed) return ctx.reply('🚫 *Sesi habis.* Silakan hubungkan ulang akun Anda.', { parse_mode: 'Markdown' });
    ...
    if (s.act === 'create_from_list') {
      ...
      if (namesAll.length > MAX_GROUPS_PER_BATCH) {
        ...
        await ctx.reply(`⚠️ *Proteksi Limit:* Hanya memproses ${MAX_GROUPS_PER_BATCH} nama pertama.`, { parse_mode: 'Markdown' });
      }

      ctx.session = null;
      await ctx.reply(`⏳ *Memproses...* Membuat ${names.length} grup. Mohon tunggu.`, { parse_mode: 'Markdown' });
      try {
        ...
        await ctx.reply(`✅ *LAPORAN SUKSES* (${res.length} Dibuat)\n\n${txt}`, opts);
      } catch (e) {
        await ctx.reply('❌ *Sistem Error:* ' + (e.message || e), { parse_mode: 'Markdown' });
      }
      return;
    }
    ...
    if (s.act === 'create_seq_name') {
      ...
      return ctx.reply(`🔢 *Masukkan Jumlah Grup:*\nKetik angka (1 - ${MAX_GROUPS_PER_BATCH})`, { 
        reply_markup: inlineCancelKb(),
        parse_mode: 'Markdown' 
      });
    }
    ...
    if (s.act === 'create_seq_count') {
      ...
      if (!Number.isFinite(count) || count < 1) {
        return ctx.reply('⚠️ *Input Tidak Valid.* Harap masukkan angka.', { reply_markup: inlineCancelKb(), parse_mode: 'Markdown' });
      }
      const safeCount = Math.min(count, MAX_GROUPS_PER_BATCH);
      if (count > MAX_GROUPS_PER_BATCH) {
        await ctx.reply(`⚠️ Limit otomatis disesuaikan ke ${MAX_GROUPS_PER_BATCH}.`);
      }
      const names = a.normalizeNamesFromPrefix(s.tmpName, safeCount);
      ctx.session = null;
      await ctx.reply(`⏳ *Memproses...* Membuat ${names.length} grup.`, { parse_mode: 'Markdown' });
      try {
        const res = await a.createManyGroupsSequential(names, { delayMs: DELAY_MS });
        const txt = res.map((x,i)=> `• ${x.title} — [Link](${x.link})`).join('\n');
        const opts = { parse_mode: 'Markdown', disable_web_page_preview: true };
